...
public class TipaniRecomendatorsController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<TipaniRecomendatorsDTO> getEntityById(@PathVariable Long id) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TipaniRecomendatorsDTO> updateEmployee(@PathVariable Long id, @RequestBody TipaniRecomendatorsDTO tipaniRecomendatorsDTO) throws ResourceNotFoundException {
        TipaniRecomendatorsDTO tipaniRecomendatorsDTO1 = service.updateTipRecom(id, tipaniRecomendatorsDTO);
        return ResponseEntity.ok(tipaniRecomendatorsDTO1);
    }
